/* ————————————————————————————————————————————————————————————————————————————————————————————————
Monte-Carlo Cross-Validation using the Bidiag2 PLS algorithm.
———————————————————————————————————————————————————————————————————————————————————————————————————
This is a MEX function which estimates the cross-validated-root-mean-squared-error, RMSEcv, by means
of randomly subsampled cross-validation. Observations with the same response (y) are always kept
together as either training or validation data. To estimate regression coefficients the function
uses a BLAS implementation of the Bidiag2 PLS algorithm described in:
...
The function takes 5 inputs (all should be double precision):
...
* Output 1: a [1 x A] vector with RMSEcv as a function of PLS component.

Example on how to compile and run from Matlab:
% Compile .C to .mexw64
>> mex -largeArrayDims -lmwblas MonteCarloCVBidiag2.c

% Run from Matlab when compiled:

>> X = rand(10000, 256);		  % Design matrix X.
>> y = rand(10000, 1);			  % Response vector y.
>> A = 10;						  % Max number of components.
>> UY = unique(y);				  % y without replicates.
>> MaxIters = 10;				  % Number of CV iterations to perform.
>> ValObs = round(length(UY)/5);  % Number of unique responses to keep as val. in each fold.

>> [ RMSEcv ] = MonteCarloCVbidiag2( X , y, A, MaxIters, ValObs );
...
void Pred(double *Xval, double *beta, double *yval, size_t n, size_t valm, int A, double *yhat, 
	double *RMSEcv, int cvIter, int MaxIters);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	int cvIter, NoValObs, MaxIters, A;
	bool *IsVal;
	...
	for (cvIter = 0; cvIter < MaxIters; cvIter++) {
		...
		Pred( Xval, beta, yval, n, valm, A, yhat, RMSEcv, cvIter, MaxIters);
...
void Pred(double *Xval, double *beta, double *yval, size_t n, size_t valm, int A, double *yhat, double *RMSEcv, int cvIter, int MaxIters) {
	...
	for (col = 0; col < A; col++) {
		...
		RMSEcv[col] += ddot(&valm, yhat + col * valm, &IntConstOne, yhat + col * valm, &IntConstOne) / valm;
		...
		if (cvIter == MaxIters - 1) {
			RMSEcv[col] = sqrt(RMSEcv[col] / MaxIters);
		}
	}
}
/* ————————————————————————————————————————————————————————————————————————————————————————————— */